/*
* $Revision: 1.1 $
* $Id: Genscan_Impl.java,v 1.1 2003/04/04 11:39:31 niclas Exp $
 *
...
/**
 * Implements the Genscan:Method interface
 * @see alfresco.corba_wrappers.genscan.Genscan
 * @version 1
 * @author Niclas Jareborg, Center for Genomics Research
 */

public class Genscan_Impl extends _GenscanImplBase implements alfresco.corba_wrappers.Method, TmpDirWriter {
 ...
 */
  public void run() {
    // Do the dirty stuff !!
    if (tmpDir == null) {
      System.out.println("alfresco.corba_wrappers.cpg.Genscan_Impl: tmpDir not set!");
    }
    FastaFile file = new FastaFile(tmpDir.getPath(), seqName, ">" + seqName, seq);
    file.write();
    alfresco.Genscan gsr = new alfresco.Genscan(file, prop);
    parseParameters();
    gsr.start();
    gsr.yield();
    try {
 ...
 */
  public GffDataStruct getGffData() throws alfresco.corba_wrappers.NoOutputException{
    if (data == null) { 
      throw new alfresco.corba_wrappers.NoOutputException("Genscan produced no output"); 
    }
    return data;
  }
/**
