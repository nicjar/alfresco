/*
* $Revision: 1.1 $
* $Id: Genewise_Impl.java,v 1.1 2003/04/04 11:39:23 niclas Exp $
 *
...
/**
 * Implements the Genewise:Method interface
 * @see alfresco.corba_wrappers.cpg.Genewise
 * @version 1
 * @author Niclas Jareborg, Center for Genomics Research
 */

public class Genewise_Impl extends _GenewiseImplBase implements alfresco.corba_wrappers.Method, TmpDirWriter {
 ...
 */
  public void input(InputStruct input) {
    ...
    this.pSeq = input.seqs[1];
    this.pSeqName = input.names[1];
  }
/**
 * Runs the analysis. 
 */
  public void run() {
    // Do the dirty stuff !!
    if (tmpDir == null) {
      System.out.println("alfresco.corba_wrappers.est_genome.Genewise_Impl: tmpDir not set!");
    }
    FastaFile gFile = new FastaFile(tmpDir.getPath(), gSeqName, ">" + gSeqName, gSeq);
    gFile.write();
    FastaFile eFile = new FastaFile(tmpDir.getPath(), pSeqName, ">" + pSeqName, pSeq);
    eFile.write();
    alfresco.Genewise egr = new alfresco.Genewise(gFile, eFile, prop);
    egr.start();
    try {
      egr.join();
      data.gff = egr.getGff();
 ...
 */
  public GffDataStruct getGffData() throws alfresco.corba_wrappers.NoOutputException{
    if (data == null) { 
      throw new alfresco.corba_wrappers.NoOutputException("Genewise produced no output"); 
    }
    return data;
  }
}
