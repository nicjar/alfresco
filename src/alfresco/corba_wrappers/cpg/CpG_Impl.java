/*
* $Revision: 1.1 $
* $Id: CpG_Impl.java,v 1.1 2003/04/04 11:39:00 niclas Exp $
 *
...
public class CpG_Impl extends _CpGImplBase implements alfresco.corba_wrappers.Method, TmpDirWriter {
 ...
 */
  public void run() {
    // Do the dirty stuff !!
    if (tmpDir == null) {
      System.out.println("alfresco.corba_wrappers.cpg.CpG_Impl: tmpDir not set!");
    }
    if (prop == null) {
      System.out.println("alfresco.corba_wrappers.cpg.CpG_Impl: Properties not set!");
    }
    FastaFile file = new FastaFile(tmpDir.getPath(), seqName, ">" + seqName, seq);
    file.write();
    parseParameters();
    alfresco.CpG cpgr = new alfresco.CpG(file, prop);
    cpgr.start();
    try {
      cpgr.join();
      data.gff = cpgr.parseToGff();
