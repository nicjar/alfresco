/*
* $Revision: 1.1 $
* $Id: BlEst_Genome_Impl.java,v 1.1 2003/04/04 11:38:45 niclas Exp $
 *
...
/**
 * Implements the BlEst_Genome:Method interface
 * @see alfresco.corba_wrappers.blest_genome.BlEst_Genome
 * @version 1
 * @author Niclas Jareborg, Center for Genomics Research
 */

public class BlEst_Genome_Impl extends _BlEst_GenomeImplBase implements alfresco.corba_wrappers.Method, TmpDirWriter {
 ...
 */
  public void run() {
    // Do the dirty stuff !!
    if (tmpDir == null) {
      System.out.println("alfresco.corba_wrappers.blest_genome.BlEstGenome_Impl: tmpDir not set!");
    }
...
//     eFile.write();
    parseParameters();
    alfresco.BlEst_Genome blegr = new alfresco.BlEst_Genome(gFile, prop);
    blegr.start();
    try {
      blegr.join();
      data.gff = blegr.getGff();
 ...
 */
  public GffDataStruct getGffData() throws alfresco.corba_wrappers.NoOutputException{
    if (data == null) { 
      throw new alfresco.corba_wrappers.NoOutputException("BlEst_Genome produced no output"); 
    }
    return data;
  }
/**
