/*
* $Revision: 1.1 $
* $Id: BlastAlign_Impl.java,v 1.1 2003/04/04 11:38:29 niclas Exp $
 *
...
/**
 * Implements the BlastAlign interface
 * @see alfresco.corba_wrappers.blastalign.BlastAlign
 * @version 1
 * @author Niclas Jareborg, Center for Genomics Research
 */

public class BlastAlign_Impl extends _BlastAlignImplBase implements TmpDirWriter{
	String seq1;
	String seq2;
	String name1;
	String name2;
  ...
  Properties prop;
	...
	/**
	 * Creates new BlastAlign_Impl
	 */
// 	public BlastAlign_Impl(File dir) {
// 		super();
// 		writedir = dir;
// 	}
	public BlastAlign_Impl() {
    String [] supl = { "" };
		result = new GffDataStruct("", "", supl);
	}
...
// 		f2 = new FastaFile(dir.getPath(), tmpfn2, ">tmp_file_2", seq2);
    f1 = new FastaFile(dir.getPath(), name1, ">" + name1, seq1);
		f2 = new FastaFile(dir.getPath(), name2, ">t" + name2, seq2);
		f1.write();
		f2.write();
		System.out.println("BlastAlign_Impl: files saved");
    alfresco.BlastAlign bla = new alfresco.BlastAlign(f1, f2, prop);
    bla.start();
    bla.yield();
    try {
      bla.join();
...
// 		try {
// 			String formatdb = "formatdb -p F -o T -t tmp -i " + f1.getPath();
// 			System.out.println("BlastAlign_Impl: running formatdb");
 ...
 */
  public GffDataStruct getGffData() throws alfresco.corba_wrappers.NoOutputException{
    if (result == null) { 
      throw new alfresco.corba_wrappers.NoOutputException("BlastAlign produced no output"); 
    }
    return result;
  }

//   /**
//    * Runs blastall with the specified method
...
//     File resFile = new File(dir, "tmpres" + pseudorand);
//     Runtime rt = Runtime.getRuntime();
//     try {
//       String blast = "blastall -p " + program + " -i " + query + " -d " + database + " -o " + resFile.getPath();
...
// 		  String mspc = "MSPcrunch -d " + blastresult.getPath();
